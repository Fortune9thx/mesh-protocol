...
import { EscrowList } from "@/components/surfaces/EscrowList";
import { OnboardingChecklist } from "@/components/surfaces/OnboardingChecklist";
  ...
  return (
    <div className="min-h-screen bg-[#0C0C0E] font-sans text-[14px] text-[--mesh-white]">
      ...
      <main className="relative mx-auto max-w-[1180px] px-7 py-9">
        ...
        {/* ── Onboarding: dashboard is guidance, not just data, when Mesh is empty ── */}
        {agents.length === 0 && (
          <div className="relative mb-4">
            <OnboardingChecklist connected={!!address} />
          </div>
        )}
        ...
        {/* ── Your escrows — dispute entry point ── */}
        <div className="mt-4">
          <EscrowList />
        </div>
        ...
        <div className="mt-4 rounded-xl border border-[#191920] bg-[#0C0C0E] px-5 py-1.5">
          ...
          {events.length === 0 ? (
            <div className="py-1">
              {[
                agents.length === 0 ? "Network awaiting participants" : "Network awaiting opportunities",
                "No disputes currently active",
                "Consensus operating normally",
                "Agent economy stable",
              ].map((msg, i) => (
                <motion.div
                  key={msg}
                  initial={{ opacity: 0 }}
                  ...
                  className="flex items-center gap-3 border-b border-[#191920] py-2.5 text-[12.5px] text-[#6B6B74] last:border-b-0"
                >
                  <span className="h-[5px] w-[5px] flex-none animate-pulse rounded-full bg-[#2E2E38]" />
                  <span>{msg}</span>
                </motion.div>
              ))}
            </div>
