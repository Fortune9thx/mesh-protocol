...
import { SuccessScreen } from "@/components/surfaces/SuccessScreen";
...
export function SubmitIntentModal({ onClose, onSubmitted }: { onClose: () => void; onSubmitted?: () => void }) {
  ...
  const [done, setDone] = useState<null | { intentId: string; hash?: string }>(null);
  ...
  const handleSubmit = async () => {
    ...
    if (result.ok) {
      onSubmitted?.();
      setDone({
        intentId: (result.data as { intent_id: string })?.intent_id ?? "",
        ...
        <div className="w-[540px] bg-graphite border border-white/18">
          <SuccessScreen
            ...
            facts={[
              ["Status", "Open"],
              ["Title", values.title || "Untitled Intent"],
              ["Budget", `${values.budget || "0"} GEN`],
              ["Priority", (values.priority || "medium").toUpperCase()],
              ["Intent ID", done.intentId],
            ]}
            hash={done.hash}
            actions={[
              { label: "Start Negotiation", primary: true, onClick: () => { onClose(); router.push("/console"); } },
              { label: "View on Network", onClick: () => { onClose(); router.push("/network"); } },
              { label: "Create Another", onClick: () => { setDone(null); setValues({}); } },
            ]}
          />
        </div>
      </ModalOverlay>
  ...
  if (submitting) {
    return (
      <ModalOverlay onClose={onClose}>
        <div className="w-[540px] bg-graphite border border-white/18">
          <div className="flex items-center px-6 py-5 border-b border-white/8">
            <div>
              <div className="font-mono text-[9.5px] tracking-[0.14em] uppercase text-[#8a8a86]">INTENT REGISTRY · CONFIRMING</div>
              <div className="text-[18px] font-extrabold mt-1">Publishing to GenLayer</div>
            </div>
          </div>
          <div className="px-6 py-8 min-h-[220px]">
            <ProtocolNarrative stageKey="intent" running />
            <p className="mt-4 text-[11.5px] text-[#6B6B74]">Your intent is committing on-chain and becoming discoverable to agents. ~15–30s.</p>
          </div>
        </div>
      </ModalOverlay>
    );
