...
export function RegisterAgentModal({ onClose, onRegistered }: { onClose: () => void; onRegistered?: () => void }) {
  ...
  const [done, setDone] = useState<null | { agentId: string; hash?: string }>(null);
  ...
  const handleSubmit = async () => {
    ...
    if (result.ok) {
      onRegistered?.();
      setDone({
        agentId: (result.data as { agent_id: string })?.agent_id ?? "",
        ...
        <div className="w-[560px] bg-graphite border border-white/18">
          <SuccessScreen
            ...
            facts={[
              ...
              ["Status", "Active"],
              ["Category", values.category || "general"],
              ["Agent ID", done.agentId],
            ]}
            hash={done.hash}
            actions={[
              { label: "Create Intent", primary: true, onClick: () => { onClose(); router.push("/console"); } },
              { label: "View Agents", onClick: () => { onClose(); router.push("/agents"); } },
              { label: "Register Another", onClick: () => { setDone(null); setStep(1); setValues({}); } },
            ]}
          />
        </div>
      </ModalOverlay>
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="w-[560px] bg-graphite border border-white/18">
        <div className="flex justify-between items-center px-6 py-5 border-b border-white/8">
          <div>
            <div className="font-mono text-[9.5px] tracking-[0.14em] uppercase text-[#8a8a86]">
              REGISTER AGENT · {submitting ? "CONFIRMING" : `STEP ${step} OF 3`}
            </div>
            <div className="text-[18px] font-extrabold mt-1">
              {submitting ? "Writing to GenLayer" : stepTitles[step]}
            </div>
          </div>
          {!submitting && (
        ...
        {/* ── Submitting: protocol narrative instead of a spinner ── */}
        {submitting ? (
          <div className="px-6 py-8 min-h-[260px]">
            <ProtocolNarrative stageKey="register" running={submitting} />
            <p className="mt-4 text-[11.5px] text-[#6B6B74]">
              GenLayer validators are reaching consensus on your agent&apos;s identity. This takes ~15–30s.
            </p>
          </div>
        ) : (
        <>
