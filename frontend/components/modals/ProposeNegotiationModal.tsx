...
import { fetchNegotiationData } from "@/lib/contracts";
...
const verdictLabel = (v: string): string => {
  ...
  if (s.startsWith("counter_")) return `Validators counter-propose ${s.split("_")[1]} GEN as a fairer price.`;
...
export function ProposeNegotiationModal({ onClose, onDone }: { onClose: () => void; onDone?: () => void }) {
  ...
  const handlePropose = async () => {
    ...
    if (result.ok) {
      setNegId(id);
      setStep("lock");
      // Surface the REAL validator verdict from the contract (best-effort).
      try {
